...
static void calc_num_rows_cols(const vec_t *locs, max_lens_t *maxlens, const struct winsize *ws,
			       int *num_rows, int *num_cols, const size_t flags);
...
void disp_basic(vec_t *locs, const struct winsize *ws, max_lens_t *maxlens, const size_t flags)
{
	...
	for(int r = 0; r < rows; ++r) {
		for(int c = 0; c < cols; ++c) {
			...
			if(S_ISDIR(stats->lnk_st.st_mode)) {
				disp(stdout, "{b}%s%-*s{0}", stats->icon,
				     c == cols - 1 ? -1 : stats->width, stats->name);
			} else if(S_ISLNK(stats->st.st_mode)) {
				if(stats->lnk_is_dead)
					disp(stdout, "{r}%s%-*s{0}", stats->icon,
					     c == cols - 1 ? -1 : stats->width, stats->name);
				else
					disp(stdout, "{y}%s%-*s{0}", stats->icon,
					     c == cols - 1 ? -1 : stats->width, stats->name);
			} else {
				disp(stdout, "{g}%s%-*s{0}", stats->icon,
				     c == cols - 1 ? -1 : stats->width, stats->name);
			}
		}
		disp(stdout, "\n");
...
begin:
	...
	if(can_reduce_rows != 0 && *num_rows > 1 &&
	   (((*num_rows) - 1) * ((*num_cols) + 1) >= count))
	{
		can_reduce_rows = 1;
		++(*num_cols);
		--(*num_rows);
