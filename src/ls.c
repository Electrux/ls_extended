...
static vec_t *generate_file_vec(DIR *dir, max_lens_t *maxlens, const char *base_loc,
				const size_t flags);
...
int ls(const struct winsize *ws, const char *loc, size_t flags, int loc_count)
{
	...
	if(strlen(loc) + 1 > MAX_STR_LEN) {
		disp(stderr, "error: too long location string length (max %d characters)",
		     MAX_STR_LEN - 1);
		return EXIT_FAILURE;
	}
	...
	if(!S_ISDIR(tmp_st.st_mode)) {
		...
		if(!(flags & OPT_N)) {
			if(S_ISDIR(stats.lnk_st.st_mode)) {
				strcpy(stats.icon,
				       get_dir_icon(final_loc, S_ISLNK(stats.st.st_mode)));
			} else {
				strcpy(stats.icon, get_file_icon(stats.name, stats.ext,
								 S_ISLNK(stats.st.st_mode)));
			}
			strcat(stats.icon, " ");
				...
				const size_t flags)
{
	...
	while((di = readdir(dir)) != NULL) {
		...
		if(!(flags & OPT_N)) {
			if(S_ISDIR(stats.lnk_st.st_mode)) {
				strcpy(stats.icon,
				       get_dir_icon(di->d_name, S_ISLNK(stats.st.st_mode)));
			} else {
				strcpy(stats.icon, get_file_icon(stats.name, stats.ext,
								 S_ISLNK(stats.st.st_mode)));
			}
		...
		if(maxlens->name < stats.namelen + stats.iconlen)
			maxlens->name = stats.namelen + stats.iconlen;
	...
	if(flags & OPT_S) {
		...
		for(int i = 0; i < vec_count(locs); ++i) vec_add(dir_locs, vec_get_data(locs, i));
		vec_destroy(&locs);
		locs	 = dir_locs;
		dir_locs = NULL;
	}
