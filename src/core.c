...
void split_file(const char *file, char *name, char *ext)
{
	...
	if(encountered_dot >= 0) {
		ctr = 0;
		for(int i = encountered_dot + 1; i < file_len; ++i) ext[ctr++] = file[i];
		ext[ctr] = '\0';
	}
...
uint8_t extra_space_count(const char *str, const int used_bytes)
{
	...
	// CJK Radicals Supplement, Kangxi Radicals, Ideographic Description Characters, CJK Symbols
	// and Punctuation, Hiragana, Katakana, Bopomofo, Hangul Compatibility Jamo, Kanbun,
	// Bopomofo Extended, Katakana Phonetic Extensions, Enclosed CJK Letters and Months, CJK
	// Compatibility, CJK Unified Ideogprahs Extension A, Yijing Hexagram Symbols, CJK Unified
	// Ideographs, Yi Syllables, Yi Radicals: 2E80 - A4CF
